...
<article class="glass-card group flex h-full flex-col gap-4 rounded-xl p-4 text-slate-200 transition-card card-glow">
    ...
    @if($school->description)
        <p class="text-sm leading-6 text-slate-300 line-clamp-3">{{ $school->description }}</p>
    @endif

    <div class="grid gap-3 sm:grid-cols-2">
        <div class="rounded-lg border border-slate-600/60 bg-slate-900/45 p-2.5">
    ...
    @if($curricula->count() || $activities->count() || $languages->count())
        <div class="mt-auto space-y-2.5 border-t border-slate-700/60 pt-3">
            @if($curricula->count())
                <div>
    ...
    @if($favoriteControls)
        <div class="{{ $curricula->count() || $activities->count() || $languages->count() ? '' : 'mt-auto ' }}flex flex-col gap-3 border-t border-slate-700/60 pt-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <form method="POST" action="{{ route('favorites.destroy', $school) }}">
                ...
                @method('DELETE')
                <button type="submit"
                        class="inline-flex min-h-9 w-full items-center justify-center gap-2 rounded-lg border border-rose-300/35 bg-rose-400/10 px-3.5 py-1.5 text-sm font-semibold text-rose-200 transition-all duration-200 hover:border-rose-300/70 hover:bg-rose-400/15 sm:w-auto">
